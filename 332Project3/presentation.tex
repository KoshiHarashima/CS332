...
\begin{frame}{Part 1 Results: Flexible(OPT lr) vs. Flexible(OPT lr)}
  \begin{minipage}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{332Project3/figures/ew_vs_ew/avg_1round_regret_convergence.png}
    \vspace{0.8em}
    \small Regret per round
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{332Project3/figures/ew_vs_3x_ew/bid_evolution.png}
    \vspace{0.8em}
    \small Varying values
  \end{minipage}\hfill
    %%% TAKEAWAYS HERE %%%
    \begin{itemize}
        \item Regret of both players converge to 0
    \end{itemize}
\end{frame}
...
\begin{frame}{Part 1 Results: Flexible (3x OPT lr) vs. Flexible (OPT lr)}
  ...
  \end{minipage}
    ...
    \begin{itemize}
        ...
        \item Player 1 = opt lr, Player 2 = 3 x opt lr
    \end{itemize}
\end{frame}
...
\begin{frame}{Part 1 Results: Myopic vs Flexible (OPT lr)}
  ...
  \end{minipage}
    %%% TAKEAWAYS HERE %%%
    \begin{itemize}
        \item Player 1 = myopic, Player 2 = flexible
        \item Myopic achieves lower regret and higher utility across MC runs
...
\begin{frame}{Part 1 Results: Myopic vs Flexible (3x OPT lr)}
  \begin{minipage}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{332Project3/figures/empirical_vs_3x_ew/bid_evolution.png}
    \vspace{0.3em}
    \small Bid
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\linewidth}
  ...
  \end{minipage}
    ...
    \begin{itemize}
        \item Player 1 = myopic, Player 2 = flexible
        \item Myopic still achieves lower regret and higher utility across MC runs
        \item Varying values yield the same.
...
Goal is to design an algorithm that can effectively exploit a No-regret learner (Exponential Weights).
Even when the opponent’s value is much higher.
we test whether strategic manipulation can still create winning opportunities.
    \begin{itemize}
        \item Parameters we use (same as Part 1)
        \begin{itemize}
            \item Rounds and runs: $n=10000$, $n_{mc}=100$
        ...
        \begin{itemize}
            \item Opponent to be exploited will use Flexible algorithm(Exponential Weight Algorithm)
            \item \textbf{Our exploitation Strategy}:
            \begin{itemize}
                \item Observe and track opponents bid for initial rounds (e.g., bid $0.2v$) .
                \item predicts when opponent bid drops below a threshold we set (e.g., bid $0.5v$) .
                \item When opponent's bid is predicted below threshold, bid to win with positive margin.
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Part 2 Results: Exploiting Fixed-Lowering Opponent}
  ...
  \end{minipage}
    %%% TAKEAWAYS HERE %%%
    \begin{itemize}
        \item Blue line is EW Algorithm and orange line is Exploitation Algorithm.
        \item At first glance, it seems to be similar to the setting of Flexible versus Flexible (EW) with different values - one always wins, and the other loses.
    \end{itemize}
\end{frame}

\begin{frame}{Part 2 Results: Exploiting Fixed-Lowering Opponent (Closer look}
  \begin{minipage}[t]{0.48\linewidth}
    \centering
  ...
  \end{minipage}
  \begin{itemize}
    \item Orange (exploiter) sometimes wins when its threshold is triggered -> After that, blue line reacts to it and raises his bid. 
    \item As for regret, opponents have to wait and observe opponent's bid while Blue line's regret converge to 0.
...
\section{Supplementary : Mixed Nash Equilibrium}
\begin{frame}{Suppelementary : Battle of Sexes}
We talked about pure Nash Equilibrium, but there is a game in which both pure and mixed Nash Equilibrium exist. Battle of sex is one of such games.
  \begin{itemize}
...
\begin{frame}{Suppelemtary : Results}
  \textbf{Results:}
      \begin{center}
        ...
        \small \textit{Flexible vs Empirical (statistic in Monte Carlo)}
        \end{minipage}
        \hfill
        \begin{minipage}[t]{0.48\linewidth}
          \centering
        ...
        \small \textit{Flexible vs Empirical (how to converge)}
        \end{minipage}
      \end{center}
    \textbf{Takeaways:}
    \begin{itemize}
...
\begin{frame}{Suppelemtary : Results}
  ...
  \begin{center}
    \begin{minipage}[t]{0.48\linewidth}
      ...
      \\[4pt]
    \small \textit{Flexible vs Flexible in Monte Carlo(statistic in Monte Carlo)}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\linewidth}
      \centering
      ...
      \\[4pt]
    \small \textit{Flexible vs Flexible (how to converge)}
    \end{minipage}
  \end{center}
  \textbf{Takeaways:}
  \begin{itemize}
